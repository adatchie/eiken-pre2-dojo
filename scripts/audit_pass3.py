...
TARGETS = ["dd05","dd09","ca08","bb07","bb08"]

def judge(name, img_path):
    ...
    prompt = (
        ...
        "OK=旧日本海軍の軍艦の古い白黒写真。NG_SHIP=軍艦だが別物・他国艦。"
        "NG_NOT_SHIP=軍艦でない（風景・人物・記念碑・地図・ポストカードの絵など）。NO_PHOTO=絵・イラストのみ。\n"
...
for sid in TARGETS:
    name = audit[sid]["name"]
    img_path = None
    for p in (BASE / "img/ships").glob(f"{sid}.*"):
        img_path = p
    if not img_path:
        print(f"{sid} {name}: NO FILE")
        continue
    for attempt in range(3):
        try:
            verdict, desc = judge(name, img_path)
            print(f"{sid} {name}: {verdict} | {desc[:80]}", flush=True)
            audit[sid]["verdict"] = verdict
            audit[sid]["desc"] = desc[:150]
            break
        except urllib.error.HTTPError as e:
            print(f"{sid}: HTTP {e.code}", flush=True)
